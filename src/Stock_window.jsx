...
export default function Stock_window({
  ...
  const openPopup = (url) => {
    if (url) {
      window.open(
        url,
        '_blank',
        'toolbar=no,menubar=no,scrollbars=yes,resizable=yes,top=100,left=200,width=1000,height=700'
      );
    }
  };

  const rawTarget = (name || ticker || '').trim();
  const safeTarget = sanitizeKey(rawTarget);
  ...
  const fetchStockData = useCallback(async () => {
    if (isFrozen || !rawTarget) return;

    const timestamp = Date.now();
    const candidateKeys = [rawTarget, safeTarget, ticker, sanitizeKey(ticker)].filter(Boolean);
    ...
    try {
      ...
      // 1. Fetch parameters (probe candidate keys)
      for (const key of candidateKeys) {
        if (!data) {
          const pRes = await fetch(`${FIREBASE_DB_URL}/param/${encodeURIComponent(key)}.json?_=${timestamp}`);
          if (pRes.ok) {
        ...
        if (!c0) {
          const c0Res = await fetch(`${FIREBASE_DB_URL}/stocks/${encodeURIComponent(key)}/0.json?_=${timestamp}`);
          if (c0Res.ok) {
        ...
        if (!c1) {
          const c1Res = await fetch(`${FIREBASE_DB_URL}/stocks/${encodeURIComponent(key)}/1.json?_=${timestamp}`);
          if (c1Res.ok) {
      ...
      // Robust CMP resolution: c0 -> data.CMP -> c1
      const currentCmp = Number(c0?.close ?? c0?.c ?? data?.CMP ?? c1?.close ?? c1?.c ?? 0);

      setFastData({
        CMP: currentCmp,
        Tdy_chng: Number(data?.['2dy-%chng'] ?? data?.['%Chg (T)'] ?? data?.['Tdy-%chng'] ?? data?.tdyChange ?? 0),
      ...
      console.error(`Error loading stock data for ${rawTarget}:`, error);
    }
  }, [rawTarget, safeTarget, ticker, isFrozen]);
  ...
  const renderStars = () => {
    ...
    if (review && review.includes('STAR')) {
      starCount = parseInt(review.split(' ')[0], 10) || 0;
    }
    return (
      <span style={{ fontSize: '18px', letterSpacing: '2px' }}>
        {[1, 2, 3, 4, 5].map((num) => (
          <span key={num} style={{ color: num <= starCount ? '#FFD700' : '#A0AAB5' }}>
            ★
          </span>
        ))}
      </span>
    ...
    >
      ...
      >
        ...
        >
          {['Tdy-%chng', 'Ydy-%chng', '1W', '1M', '3M', '1YR', '3YR', 'RSI'].map((head, idx) => (
            <div
              ...
              style={{
                ...
                borderRight: idx < 7 ? '1px solid #444' : 'none',
          ...
          <div style={{ padding: '10px 2px', borderRight: '1px solid #444', fontSize: '14px', fontWeight: 'bold', color: fastData.Tdy_chng >= 0 ? COLOR_GREEN : COLOR_RED }}>
            {fastData.Tdy_chng}%
          </div>
          <div style={{ padding: '10px 2px', borderRight: '1px solid #444', fontSize: '14px', fontWeight: 'bold', color: fastData.Ydy_chng >= 0 ? COLOR_GREEN : COLOR_RED }}>
            {fastData.Ydy_chng}%
          </div>
          <div style={{ padding: '10px 2px', borderRight: '1px solid #444', fontSize: '14px', fontWeight: 'bold', color: slowData['1W'] >= 0 ? COLOR_GREEN : COLOR_RED }}>
            {slowData['1W']}%
          </div>
          <div style={{ padding: '10px 2px', borderRight: '1px solid #444', fontSize: '14px', fontWeight: 'bold', color: slowData['1M'] >= 0 ? COLOR_GREEN : COLOR_RED }}>
            {slowData['1M']}%
          </div>
          <div style={{ padding: '10px 2px', borderRight: '1px solid #444', fontSize: '14px', fontWeight: 'bold', color: slowData['3M'] >= 0 ? COLOR_GREEN : COLOR_RED }}>
            {slowData['3M']}%
          </div>
          <div style={{ padding: '10px 2px', borderRight: '1px solid #444', fontSize: '14px', fontWeight: 'bold', color: slowData['1YR'] >= 0 ? COLOR_GREEN : COLOR_RED }}>
            {slowData['1YR']}%
          </div>
          <div style={{ padding: '10px 2px', borderRight: '1px solid #444', fontSize: '14px', fontWeight: 'bold', color: slowData['3YR'] >= 0 ? COLOR_GREEN : COLOR_RED }}>
            {slowData['3YR']}%
          </div>
          <div style={{ padding: '10px 2px', fontSize: '14px', fontWeight: 'bold', color: slowData.RSI > 50 ? COLOR_GREEN : COLOR_RED }}>
            {slowData.RSI}
          </div>
        </div>
        ...
        <div style={{ display: 'flex', width: '100%', alignItems: 'center' }}>
          <div style={{ width: '85%', display: 'flex', flexDirection: 'column', paddingRight: '15px', borderRight: '1px dashed #444' }}>
            <div style={{ position: 'relative', height: '110px', width: '100%' }}>
              <div style={{ position: 'absolute', left: 0, top: '50%', transform: 'translateY(-50%)', fontWeight: 'bold', fontSize: '12px', color: '#cccccc' }}>
                52WL
              </div>
              <div style={{ position: 'absolute', right: 0, top: '50%', transform: 'translateY(-50%)', fontWeight: 'bold', fontSize: '12px', color: '#cccccc' }}>
                52WH
              </div>
      ...
      {/* RIGHT: 30% FUNDAMENTAL EXTENSION PANEL */}
      <div
        style={{
          flex: '3',
          position: 'relative',
      ...
      >
        <div style={{ display: 'flex', justifyContent: 'space-between', fontWeight: 'bold', fontSize: '13px' }}>
          <div>
            <span style={{ color: GRP_VALUATION }}>PE:</span> <span>{pe}</span> (
            <span style={{ color: dpe < 0 ? COLOR_RED : COLOR_GREEN }}>{dpe}%</span>)
          </div>
          <div>
            <span style={{ color: GRP_VALUATION }}>PB:</span> <span>{pb}</span> (
            <span style={{ color: dpb < 0 ? COLOR_RED : COLOR_GREEN }}>{dpb}%</span>)
          </div>
        </div>
        <div style={{ display: 'flex', justifyContent: 'space-between', fontWeight: 'bold', fontSize: '13px' }}>
          <div>
            <span style={{ color: GRP_VALUATION }}>PS:</span> <span>{ps}</span>
          </div>
          <div>
            <span style={{ color: GRP_VALUATION }}>D. Yield:</span> <span>{dy}%</span>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', fontWeight: 'bold', fontSize: '13px' }}>
          <div>
            <span style={{ color: GRP_SCORES }}>T-Scr:</span> <span>{tScore}</span>
          </div>
          <div>
            <span style={{ color: GRP_SCORES }}>F-Scr:</span> <span>{fScore}</span>
          </div>
          <div>
            <span style={{ color: GRP_SCORES }}>G-Scr:</span> <span>{gScore}</span>
          </div>
        </div>
      ...
      {isRemarkOpen && (
        ...
        >
          ...
          >
            ...
            <h2 style={{ marginTop: 0, color: '#FFD700', borderBottom: '1px solid #444', paddingBottom: '10px' }}>
              Remark: {name}
            </h2>
            <p style={{ color: '#ffffff', fontSize: '16px', lineHeight: '1.6' }}>
              {remark || 'No remarks available for this stock.'}
            </p>
          </div>
        </div>
      )}
