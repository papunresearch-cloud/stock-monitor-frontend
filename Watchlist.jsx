...
// Formatted strictly with hyphens (DD-MM-YYYY) like pre-integration
...
export default function StockWatchlist() {
  ...
  // Data Integrity Pipeline (Ensures screener fundamentals & identifiers merge smoothly)
  useEffect(() => {
    ...
    setStockDatabase(prev => {
      ...
      watchlistNames.forEach(rawItem => {
        ...
        // Full Identifiers restored from mainRecord
        if (existing.Name !== (mainRecord.Name || nameStr)) { mergedRecord.Name = mainRecord.Name || nameStr; recordChanged = true; }
        if (existing.CODE !== (mainRecord.CODE || existing.CODE || nameStr)) { mergedRecord.CODE = mainRecord.CODE || existing.CODE || nameStr; recordChanged = true; }
        if (existing.NSE !== (mainRecord.NSE || existing.NSE || "")) { mergedRecord.NSE = mainRecord.NSE || existing.NSE || ""; recordChanged = true; }
        if (existing.BSE !== (mainRecord.BSE || existing.BSE || "")) { mergedRecord.BSE = mainRecord.BSE || existing.BSE || ""; recordChanged = true; }
        if (!existing.type) { mergedRecord.type = mainRecord.type || "STOCK"; recordChanged = true; }
        if (mainRecord.sector && existing.sector !== mainRecord.sector) { mergedRecord.sector = mainRecord.sector; recordChanged = true; }
        if (mainRecord.industry && existing.industry !== mainRecord.industry) { mergedRecord.industry = mainRecord.industry; recordChanged = true; }
        if (!existing.addedOn) { mergedRecord.addedOn = mainRecord.addedOn || formatDateToDDMMYYYY(new Date()); recordChanged = true; }
  ...
  const handleOkAddStock = async () => {
    ...
    // Build the complete pre-integration object with screener enrichment
    const enrichedStockRecord = {
      ...
      CODE: mainRecord.CODE || stockToAdd,
      NSE: mainRecord.NSE || "",
      BSE: mainRecord.BSE || "",
      type: mainRecord.type || "STOCK",
      sector: mainRecord.sector || "",
      industry: mainRecord.industry || "",
      addedOn: mainRecord.addedOn || today
    };

    // Attach screener fundamentals immediately upon adding
    APP_CONFIG.enrichmentMetrics.forEach(metric => {
      if (mainRecord[metric] !== undefined) {
        enrichedStockRecord[metric] = mainRecord[metric];
      }
    });
    ...
    try {
      ...
      // Mirror ticker in /stocklist for backend workers
        ...
        <div style={{ position: "fixed", top: 0, left: 0, width: "100vw", height: "100vh", backgroundColor: "rgba(0,0,0,0.8)", display: "flex", justifyContent: "center", alignItems: "center", zIndex: 9999, padding: "20px" }}>
          <div style={{ backgroundColor: "#0f172a", border: `3px solid ${theme.accentAmber}`, borderRadius: "12px", padding: "30px", width: "600px", maxWidth: "100%", color: "#f8fafc", boxShadow: "0 10px 40px rgba(0,0,0,0.7)" }}>
            ...
            <div style={{ fontSize: "13px", fontWeight: "bold", lineHeight: "1.6", display: "flex", flexDirection: "column", gap: "10px", marginBottom: "24px" }}>
              ...
              <p>🔹 <b>Column DATE:</b> Read Only. Auto-updates with hyphens (DD-MM-YYYY) when you Save modifications made to Core Fields.</p>
              ...
              <p style={{ color: theme.accentCyan }}>🟢 <b>Cloud Native:</b> Full metadata and screener enrichment fundamentals are saved directly to Firebase Realtime Database.</p>
            </div>
